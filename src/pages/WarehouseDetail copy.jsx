...
//   const [tab, setTab] = useState("stock"); // stock | trx
...
//   const [dateFromJ, setDateFromJ] = useState(""); // "1403/06/24"
//   const [dateToJ, setDateToJ] = useState("");     // "1403/06/25"
...
//     let list = transactions.filter((t) => t?.from_wh?.id === whId || t?.to_wh?.id === whId);
...
//   }, [transactions, whId, trxType, usage, dateFromJ, dateToJ, q]);
...
//       actions={
//         <Link to="/inv/warehouses" className="rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">
//           بازگشت
//         </Link>
//       }
//     >
...
//           <button onClick={() => setTab("stock")} className={`rounded-xl px-3 py-1.5 text-sm font-semibold ${tab === "stock" ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}>
//             موجودی
//           </button>
//           <button onClick={() => setTab("trx")} className={`rounded-xl px-3 py-1.5 text-sm font-semibold ${tab === "trx" ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}>
//             تراکنش‌ها
//           </button>
//         </div>
...
//             <input type="text" placeholder="مثلاً: MFD یا 9304..." className="rounded-xl border border-gray-200 px-3 py-2 text-sm w-full" value={q} onChange={(e) => setQ(e.target.value)} />
//           </div>
...
//                 <select className="rounded-xl border border-gray-200 px-3 py-2 text-sm w-full" value={trxType} onChange={(e) => setTrxType(e.target.value)}>
...
//                 <select className="rounded-xl border border-gray-200 px-3 py-2 text-sm w-full" value={usage} onChange={(e) => setUsage(e.target.value)}>
//                   <option value="all">همه</option>
...
//                 <input type="text" placeholder="YYYY/MM/DD" className="rounded-xl border border-gray-200 px-3 py-2 text-sm w-full ltr:text-left" value={dateFromJ} onChange={(e) => setDateFromJ(e.target.value)} />
//               </div>

//               <div className="flex flex-col">
//                 <label className="text-xs font-medium text-gray-600">تا تاریخ (شمسی)</label>
//                 <input type="text" placeholder="YYYY/MM/DD" className="rounded-xl border border-gray-200 px-3 py-2 text-sm w-full ltr:text-left" value={dateToJ} onChange={(e) => setDateToJ(e.target.value)} />
//               </div>
//             </>
//           )}
//         </div>
